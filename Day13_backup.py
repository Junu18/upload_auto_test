...
from verification import and_gate
...
@app.rout("/verilog",methods=["POST"])
def verilog_v():
    if request.method == "POST":
        f3 = request.files["verilog"]
        f3_filename = secure_filename(f3.filename)
        f3_path = url_for("static", filename="static/" + f3_filename)
        f3.save(f3_path)
        r = and_gate_verificate(f3_path)
        return r 
